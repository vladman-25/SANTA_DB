...
public final class NiceScoreStrategy implements GiftsStrategy {
    ...
    getGiftsByStrategy(final HashMap<Integer, Double> budgetMap) {
        ...
        newChildren.sort((o1, o2) -> {
            if (Double.compare(Database.getDatabase().getAvgScores().get(o1.getId()),
                    Database.getDatabase().getAvgScores().get(o2.getId())) < 0) {
                return 1;
            }
            if (Double.compare(Database.getDatabase().getAvgScores().get(o1.getId()),
                    Database.getDatabase().getAvgScores().get(o2.getId())) > 0) {
